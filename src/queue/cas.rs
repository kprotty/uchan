...
impl<T> Queue<T> {
    ...
    pub(crate) fn send(&self, value: T) -> Result<(), T> {
        ...
        loop {
            ...
            let (mut block, mut index, disconnected) = Block::<T>::decode(tail);
            if disconnected {
                return Err(value);
            }
            ...
            if let Err(_) = self.producer.compare_exchange(
                tail,
                Block::encode(next_block, next_index, false),
                Ordering::AcqRel,
                Ordering::Relaxed,
            ) {
                Backoff::yield_now();
            ...
            return Ok(unsafe {
                if block != next_block {
                    ...
                    assert_eq!(new_block, next_block);
                    new_block = null_mut();
    ...
    pub(crate) unsafe fn recv<E: Event>(&self) -> Result<T, ()> {
        loop {
            match self.try_recv() {
                ...
                Ok(None) => self.consumer.parker.park::<E>(),
            }
        }
    }
    ...
    pub(crate) fn disconnect(&self, is_sender: bool) {
        ...
        let tail = fetch_add_ptr(&*self.producer, DISCONNECT_BIT, Ordering::AcqRel);
